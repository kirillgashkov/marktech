...
local function parse_include_attributes(
  ...
  for key, value in pairs(attributes) do
    ...
    elseif key == "to" then
      ...
      if end_line_number == nil then
        io.stderr:write("Error: 'to' attribute must be an integer.\n")
        os.exit(1)
      end
    end
  end
  ...
  if include_filepath == nil and (start_line_number ~= nil or end_line_number ~= nil) then
    io.stderr:write("Warning: code block has 'from' or 'to' attribute, but no 'include' attribute.\n")
  end
...
local function validate_end_line_number(
  ...
  if end_line_number < 1 then
    io.stderr:write("Error: the end line number must be greater than 0.\n")
    os.exit(1)
  end
end
...
local function validate_start_end_line_numbers(
  ...
  if start_line_number > end_line_number then
    io.stderr:write("Error: the start line number must be less than or equal to the end line number.\n")
    os.exit(1)
  end
end
...
local function make_code_block(
  ...
  if code_block_el.text ~= "" then
    io.stderr:write("Warning: code block has 'include' attribute, but also has text. " .. "The text will be ignored.\n")
  end
