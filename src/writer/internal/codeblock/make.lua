...
local function parse_code_attributes(
  ...
  for key, value in pairs(attributes) do
    ...
    elseif key == "from" then
      ...
      if start_line_number == nil then
        io.stderr:write("Error: 'from' attribute must be an integer.\n")
        os.exit(1)
      end
    end
  end
  ...
  if should_warn_about_extra_captions then
    io.stderr:write("Warning: 'caption' and 'caption_json' attributes are both provided. Ignoring 'caption'.\n")
    io.stderr:write(
      "Hint: Check for extra 'caption' attributes in your document when you are using paragraph captions.\n"
    )
  end
...
local function validate_start_line_number(
  ...
  if start_line_number < 1 then
    io.stderr:write("Error: the start line number must be greater than 0.\n")
    os.exit(1)
  end
end
...
local function make_code_block(
  ...
  if not identifier:match("^[%a%d%-:]*$") then
    io.stderr:write("Warning: identifier contains bad characters: " .. identifier .. "\n")
    io.stderr:write("Hint: identifiers should only contain alphanumeric characters, hyphens, and colons.\n")
  end
...
local function make_code_block_main(
  code_block -- pandoc.CodeBlock
)
  ...
  if captionable:has_caption() then
    if captionable.caption ~= nil then
      code_block_to_render.attributes.caption_json = pandoc.write(pandoc.Pandoc({ captionable.caption }), "json")
    elseif captionable.id ~= nil then
      code_block_to_render.attributes.caption_json = pandoc.write(pandoc.Pandoc({}), "json")
    end
...
---@return any
function codeblock.MakeLatex(e)
  return make_code_block_main(e)
end
