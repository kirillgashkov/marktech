...
local function is_math_para(
  ...
  return (#para.content == 1 and para.content[1].t == "Math" and para.content[1].mathtype == "DisplayMath")
    or (
      #para.content == 1
...
---@param e pandoc.Para
---@return any
function math_.MakeLatexFromPara(e)
  if not is_math_para(e) then
    return e
  end

  local math_span = math_para_to_math_span(e)
  local id = get_math_span_id(math_span)
  local caption = nil -- Math does not have captions.

  local m = math_span_to_math(math_span)
